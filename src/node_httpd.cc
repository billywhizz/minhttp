...
class Socket : public ObjectWrap {
    ...
    static int body_cb (http_parser *p, const char *buf, size_t len) {
      ...
      if(len > 0) {
        ...
        Socket* s = static_cast<Socket*>(ctx->sock);
        if(s->cb.onBody) {
          //TODO: can't i avoid this copy somehow?
    ...
    static void after_write(uv_write_t* req, int status) {
      ...
      // free the bytes malloc'd in the call to Write
      free(wr->buf.base);
      free(wr);
    }
	...
	protected:
      ...
      server->cb.onResponse = server->cb.onWrite = server->cb.onError = server->cb.onClose = server->cb.onRequest = server->cb.onBody = server->cb.onHeaders = server->cb.onConnect = 0;
    ...
    static Handle<Value> BindCallbacks(const Arguments &args) {
      ...
      // need to copy bytes to written as they could be overwritten by a subsequent write before the actual bytes get written to the socket
    ...
    static Handle<Value> Connect(const Arguments& args) {
      ...
      s->tcpServer = (uv_tcp_t*)malloc(sizeof(uv_tcp_t));
      s->tcpServer->data = s;
      struct sockaddr_in addr = uv_ip4_addr(*ip_address, port);
      int r = 0;
      r = uv_tcp_init(loop, s->tcpServer);
      if (r) {
        fprintf(stderr, "Socket creation error\n");
        r = -1;
      }
      else {
        r = uv_tcp_bind(s->tcpServer, addr);
        if (r) {
          fprintf(stderr, "Bind error\n");
          r = -2;
        }
        else {
          r = uv_listen((uv_stream_t*)s->tcpServer, SOMAXCONN, on_server_connection);
          if (r) {
